...
public class MeshSubstrate extends Substrate {
	...
	public MeshSubstrate(SimulationParams simParams){
		...
		substrateBottomLeft= new double[]{Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE};
		...
		for(triIt= triangles.iterator(); triIt.hasNext(); ){
            ...
            for(int k=0; k<3; k++){
                ...
                for(int i=0; i<D; i++){
                    ...
                    if(vert[i]<substrateBottomLeft[i]){
                        substrateBottomLeft[i]=vert[i];
                    }
		...
		logger.info("Mesh dimension: "+(topRight[0]-substrateBottomLeft[0])+","+(topRight[1]-substrateBottomLeft[1])+","+(topRight[2]-substrateBottomLeft[2]));
		...
		if(SimulationParams.sim_mesh_sep != null){
			...
			for(int i=0; i<D; i++){
				if(SimulationParams.sim_mesh_sep[i]>0.0){
					...
					double L= topRight[i]-substrateBottomLeft[i];
					...
					substrateBottomLeft[i]-=halfDiff;
					topRight[i]+=halfDiff;
		...
		logger.info("bottom left = ("+substrateBottomLeft[0]+","+substrateBottomLeft[1]+","+substrateBottomLeft[2]+")");
		/* 
		...
		for(int i=0; i<D; i++){
		    ...
		    this.L[i]=topRight[i]-substrateBottomLeft[i];
		    super.L[i]= this.L[i];
	...
	public boolean crossesMembrane(Walker walker, double[] offset, double[] stepVector,
		...
		boolean crossingIsSubstrateBoundary = false;
		...
		getSubstrateCoords(walker, offset, subsCoords);

		stepVector = mapStepIntoSubstrate(walker, offset, stepVector);
		...
		if(checkBoundaryIntersection(subsCoords, offset, stepVector, tempNormal, tempD, false, origLength, intDist, null, tempP)){
            ...
            if(intDist[0]<nearest){
                ...
                walker.reflectionCount += 1;
                crossingIsSubstrateBoundary = true;
            }
		}
		...
		if (crossingIsSubstrateBoundary && SimulationParams.sim_boundary == SimulationParams.BOUNDARY_POINT_REFLECT) {
			double[] crossingPoint = new double[MeshSubstrate.D];
			for (int i = 0; i < MeshSubstrate.D; i++) {
				crossingPoint[i] = subsCoords[i] + nearest * stepVector[i];
				walker.reflectionVec[i] = 2 * crossingPoint[i] - walker.reflectionVec[i];
			}
//			logger.warning("Reflection Point: [" + crossingPoint[0] + ", " + crossingPoint[1] + ", " + crossingPoint[2] + "]");

		}
		stepVector = unmapStepFromSubstrate(walker, offset, stepVector);
	...
	/** 
	 * Overload getSubstrateCoords to move the walker into the right part of the mesh if 
	 * point reflection boundaries are used. 
	 */
    public void getSubstrateCoords(Walker walker, double[] offset, double[] newPos) {
		if (SimulationParams.sim_boundary == SimulationParams.BOUNDARY_DEFAULT) {
    			getSubstrateCoords(walker.r, offset, newPos);
		}
		else if (SimulationParams.sim_boundary == SimulationParams.BOUNDARY_POINT_REFLECT) {
			for (int i = 0; i < MeshSubstrate.D; i++) {
				newPos[i] = walker.r[i] + offset[i];
				newPos[i] = walker.reflectionVec[i] + Math.pow(-1, walker.reflectionCount) * newPos[i];
			}
     ...
     * For reflective boundary conditions, the step gets flipped based on number of reflections
     * 
     * 
    ...
    public double[] mapStepIntoSubstrate(Walker walker, double[] offset, double[] step){
    		...
    		double[] newStep = new double[MeshSubstrate.D];
        if (SimulationParams.sim_boundary == SimulationParams.BOUNDARY_POINT_REFLECT) {
        	
        		for (int i = 0; i < MeshSubstrate.D; i++) {
        			newStep[i] = Math.pow(-1, walker.reflectionCount) * step[i];
        		}
        }
        else {
        		for (int i = 0; i < MeshSubstrate.D; i++) {
     ...
     */
    public double[] unmapStepFromSubstrate(Walker walker, double[] offset, double[] step){
		double[] newStep = new double[MeshSubstrate.D];
        if (SimulationParams.sim_boundary == SimulationParams.BOUNDARY_POINT_REFLECT) {
