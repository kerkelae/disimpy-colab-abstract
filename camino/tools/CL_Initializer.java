		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("-initial")) {
				if (args[i + 1].equalsIgnoreCase("uniform")) {
					...
					SimulationParams.sim_initial = SimulationParams.FILE;
					markAsParsed(i + 1);
				}
				else {
			...
			if (args[i].equalsIgnoreCase("-boundary")) {
				if (args[i+1].equalsIgnoreCase("default")){
					SimulationParams.sim_boundary = SimulationParams.BOUNDARY_DEFAULT;
					markAsParsed(i+1);
				}
				else if (args[i+1].equalsIgnoreCase("pointreflect")){
					SimulationParams.sim_boundary = SimulationParams.BOUNDARY_POINT_REFLECT;
					markAsParsed(i+1);
				}
				else {
					SimulationParams.sim_boundary = SimulationParams.BOUNDARY_DEFAULT;
					logger.warning("Boundary condition \'" + args[i+1] + "\' not recognised. Using default boundary condition" );
					markAsParsed(i+1);
				}
				brownianSimulation = true;
				markAsParsed(i);
            ...
            if (args[i].equals("-initfile")) {
    				SimulationParams.initFile = new String(args[i + 1]);
    				String suffix = SimulationParams.initFile
    						.substring(SimulationParams.initFile.length() - 4);
    				if (!suffix.equalsIgnoreCase(".dat")) {
    					SimulationParams.initFile += ".dat";
    				}            		
            		markAsParsed(i, 2);
            }
            if (args[i].equals("-phasefile")) {
            		SimulationParams.sim_statsfile = new String(args[i+1]);
            		SimulationParams.sim_StatsModType = StatsModuleType.PHASE;
            		markAsParsed(i, 2);
            }
